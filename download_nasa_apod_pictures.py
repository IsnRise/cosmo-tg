...
def fetch_nasa_apod_pictures(args_pathargs_path, nasa_pictures_count, nasa_token):
    ...
    for number, nasa_link in enumerate(nasa_apod_response_links, start=1):
        ...
        nasa_apod_params = {}
        helper_script.download_picture(nasa_apod_response, nasa_link["url"], nasa_apod_params, args_pathargs_path, nasa_apod_filename)
...
def main():
    ...
    args_pathargs_path = os.path.abspath(args.path)
    Path(args_pathargs_path).mkdir(parents=True, exist_ok=True)
    try:
        fetch_nasa_apod_pictures(args_pathargs_path, nasa_pictures_count, nasa_token)
