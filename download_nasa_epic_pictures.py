...
def fetch_nasa_epic_pictures(args_pathargs_path,  nasa_token):
    ...
    for number, nasa_epic_item in enumerate(nasa_epic_response, start=1):
        ...
        helper_script.download_picture(nasa_epic_response, nasa_epic_params, nasa_epic_link, args_pathargs_path, nasa_epic_filename)
...
def main():
    ...
    args_pathargs_path = args.path
    Path(args_pathargs_path).mkdir(parents=True, exist_ok=True)
    try:
        fetch_nasa_epic_pictures(args_pathargs_path, nasa_token)
